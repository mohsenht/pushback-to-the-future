...
class TestBusyETDExtractor(TestCase):
    ...
    def test_load_data_with_regular_input(self):
        ...
        loaded_data = BusyETDExtractor().load_data(
            now,
            loaded_data,
            input_data,
            type_container
        ...
        expected_loaded_data = cudf.read_csv(
            f"data{SEPARATOR}BusyETDExtractor{SEPARATOR}expected_data.csv",
            parse_dates=[COLUMN_NAME_TIMESTAMP]
        ).sort_values(COLUMN_NAME_TIMESTAMP)
        ...
        assert expected_loaded_data.equals(loaded_data)
